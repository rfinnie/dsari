...
# SPDX-PackageDownloadLocation: https://forge.colobox.com/rfinnie/dsari
...
class TestCroniterHash(TestCase):
    ...
    def test_hash_hourly(self):
        ...
        obj = croniter.croniter("H * * * *", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 0, 10))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 0, 10) + timedelta(hours=1))

    def test_hash_daily(self):
        """Test manually-defined daily"""
        obj = croniter.croniter("H H * * *", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10) + timedelta(days=1))

    def test_hash_weekly(self):
        """Test manually-defined weekly"""
        obj = croniter.croniter("H H * * H", self.epoch, hash_id=self.hash_id)
        ...
        self.assertIn(testval, (datetime(2020, 1, 3, 11, 10), datetime(2020, 1, 5, 11, 10)))
    ...
    def test_hash_monthly(self):
        ...
        obj = croniter.croniter("H H H * *", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10) + timedelta(days=31))

    def test_hash_yearly(self):
        """Test manually-defined yearly"""
        obj = croniter.croniter("H H H H *", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 9, 1, 11, 10))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 9, 1, 11, 10) + timedelta(days=365))
    ...
    def test_hash_word_midnight(self):
        ...
        """
        obj = croniter.croniter("@midnight", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 2, 10, 32))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 2, 10, 32) + timedelta(days=1))

    def test_hash_word_hourly(self):
        """Test built-in @hourly"""
        obj = croniter.croniter("@hourly", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 0, 10, 32))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 0, 10, 32) + timedelta(hours=1))

    def test_hash_word_daily(self):
        """Test built-in @daily"""
        obj = croniter.croniter("@daily", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10, 32))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10, 32) + timedelta(days=1))

    def test_hash_word_weekly(self):
        """Test built-in @weekly"""
        obj = croniter.croniter("@weekly", self.epoch, hash_id=self.hash_id)
        testval = obj.get_next(datetime)
        # croniter 1.0.5 changes the defined weekly range from (0, 6)
        # to (0, 7), to match cron's behavior that Sunday is 0 or 7.
        # This changes our hash, so test for either.
    ...
    def test_hash_word_monthly(self):
        ...
        obj = croniter.croniter("@monthly", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10, 32))
        self.assertEqual(
    ...
    def test_hash_word_yearly(self):
        ...
        obj = croniter.croniter("@yearly", self.epoch, hash_id=self.hash_id)
    ...
    def test_hash_word_annually(self):
        ...
        """
        obj_annually = croniter.croniter("@annually", self.epoch, hash_id=self.hash_id)
        obj_yearly = croniter.croniter("@yearly", self.epoch, hash_id=self.hash_id)
    ...
    def test_hash_id_change(self):
        ...
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10) + timedelta(days=1))
        obj = croniter.croniter("H H * * *", self.epoch, hash_id="different id")
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 0, 24))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 0, 24) + timedelta(days=1))
    ...
    def test_hash_epoch_change(self):
        ...
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 11, 10) + timedelta(days=1))
        obj = croniter.croniter("H H * * *", datetime(2011, 11, 11, 11, 11), hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2011, 11, 12, 11, 10))
        self.assertEqual(obj.get_next(datetime), datetime(2011, 11, 12, 11, 10) + timedelta(days=1))

    def test_hash_range(self):
        """Test a hashed range definition"""
        obj = croniter.croniter("H H H(3-5) * *", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 5, 11, 10))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 5, 11, 10) + timedelta(days=31))

    def test_hash_id_bytes(self):
        """Test hash_id as a bytes object"""
        obj = croniter.croniter("H H * * *", self.epoch, hash_id=b"\x01\x02\x03\x04")
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 14, 53))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 14, 53) + timedelta(days=1))
    ...
    def test_random(self):
        ...
        obj = croniter.croniter("R R * * *", self.epoch, hash_id=self.hash_id)
    ...
    def test_random_range(self):
        ...
        obj = croniter.croniter("R R R(10-20) * *", self.epoch, hash_id=self.hash_id)
        result_1 = obj.get_next(datetime)
    ...
    def test_cron(self):
        ...
        obj = croniter.croniter("35 6 * * *", self.epoch, hash_id=self.hash_id)
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 6, 35))
        self.assertEqual(obj.get_next(datetime), datetime(2020, 1, 1, 6, 35) + timedelta(days=1))
    ...
    def test_invalid_hash_id_type(self):
        ...
        with self.assertRaises(TypeError):
            croniter.croniter("H H * * *", self.epoch, hash_id={1: 2})
