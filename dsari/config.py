...
class ConfigLoader:
            ...
            if os.path.exists(os.path.join(config_dir, fn)):
                try:
                    config = utils.dict_merge(
                        config,
                        utils.load_structured_file(os.path.join(config_dir, fn), file_type=fn_type),
                    )
        ...
        if self.config.config_d and os.path.isdir(self.config.config_d):
            ...
            for fn_type in ["yaml", "json"]:
                ...
                for file in config_files:
                    try:
                        config = utils.dict_merge(config, utils.load_structured_file(file, file_type=fn_type))
                    except (ValueError, ImportError) as e:
                        raise ConfigError(e)
    ...
    def populate_object(self, obj, level, source, valid_values, value_transforms):
        for k, v in source.items():
            ...
            if type(v) not in valid_values[k]:
                raise ConfigError("{}: {}: Invalid value {} (expected {})".format(level, k, repr(type(v)), repr(valid_values[k])))
    ...
    def build_job(self, job_name, job_dict):
        ...
        if isinstance(job_dict["command"], str):
